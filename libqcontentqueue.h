...
public:
    HubQueue(const std::string &host, uint16_t port, const std::string &name, uint32_t timeout = 30000000) : m_hub_client(host, port, timeout), m_host(host), m_port(port), m_name(name), m_timeout(timeout) {
